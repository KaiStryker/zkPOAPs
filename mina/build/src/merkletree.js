...
import { SmartContract, isReady, Poseidon, Field, Experimental, Permissions, State, state, CircuitValue, 
// PublicKey,
// UInt64,
prop, Mina, method,
// AccountUpdate,
...
import { generateURI } from "./PinataAPI/pinToPinata";
await isReady;
// const doProofs = true;
...
export class zkPOAP extends SmartContract {
    ...
    init(initialCommitment, initialIndex) {
    ...
    proveAttendance(nullifierHash, poapId, account, path) {
    ...
    updateRoot(poapId, account, path) {
        // we fetch the on-chain commitment
        let commitment = this.commitment.get();
        this.commitment.assertEquals(commitment);
...
], zkPOAP.prototype, "commitment", void 0);
__decorate([
    state(Field),
    __metadata("design:type", Object)
], zkPOAP.prototype, "index", void 0);
__decorate([
    method,
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Field, typeof BigInt === "function" ? BigInt : Object]),
    __metadata("design:returntype", void 0)
], zkPOAP.prototype, "init", null);
__decorate([
    method,
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Field, Field, Account, MerkleWitness]),
    __metadata("design:returntype", void 0)
], zkPOAP.prototype, "proveAttendance", null);
__decorate([
    method,
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Field, Account, MerkleWitness]),
    __metadata("design:returntype", void 0)
], zkPOAP.prototype, "updateRoot", null);
/////// set up environment \\\\\\\
...
// tx.send();
/////// set up environment \\\\\\\
export async function checkAttendance(digest, signature, tokenId, poapId, nullifierHash, zkPOAPApp, Tree, feePayer, zkAppKey, doProofs) {
    let ethAddress = new Account(ethers.utils.recoverAddress(digest, signature));
...
export async function updateMerkle(digest, signature, poapId, zkPOAPApp, Tree, feePayer, zkAppKey, doProofs) {
    let ethAddress = new Account(ethers.utils.recoverAddress(digest, signature));
    let index = zkPOAPApp.index.get();
    let w = Tree.getWitness(index);
    let witness = new MerkleWitness(w);
    let nullifier_ = Poseidon.hash([Field(poapId).add(Field(index))]);
    let tx = await Mina.transaction(feePayer, () => {
    ...
    let tokenURI = await generateURI(zkPOAP);
